...
def save_device_id_to_cookie(device_id):
    """기기 코드를 브라우저 쿠키에 저장"""
    cookie_manager = get_cookie_manager()
    if cookie_manager:
        cookie_manager.set("writey_device_id", device_id, expires_at=datetime(2030, 1, 1))

def load_device_id_from_cookie():
    """브라우저 쿠키에서 기기 코드 로드"""
    cookie_manager = get_cookie_manager()
    if cookie_manager:
        return cookie_manager.get("writey_device_id")
    return None

def get_device_id():
    """기기 코드 반환 (세션 > 쿠키 순서로 확인)"""
    # 1. 세션에 있으면 반환
    if 'browser_device_id' in st.session_state and st.session_state['browser_device_id']:
        return st.session_state['browser_device_id']
    # 2. 쿠키에서 로드 시도
    cookie_device_id = load_device_id_from_cookie()
    if cookie_device_id:
        st.session_state['browser_device_id'] = cookie_device_id
        ...
        if not device_id:
            ...
            with tab1:
                ...
                if st.button("확인", key="btn_manual_id"):
                    if manual_id:
                        ...
                        save_device_id_to_cookie(manual_id)  # 쿠키에 저장
                        st.rerun()
            ...
            with tab2:
                ...
                if st.button("이 코드로 등록 요청", key="btn_new_device"):
                    ...
                    save_device_id_to_cookie(new_code)  # 쿠키에 저장
