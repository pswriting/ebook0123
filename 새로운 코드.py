...
# 브라우저 localStorage 사용 (기기별 로그인 정보 저장)
try:
...
# 설정
# ==========================================
# 주의: Streamlit Cloud에서는 로컬 파일이 모든 사용자에게 공유됨
# 따라서 API 키, 비밀번호 등은 JSONBin에 기기별로 저장
# ==========================================
def get_config_path():
...
def save_api_key(api_key):
    """JSONBin에 기기별 API 키 저장"""
    device_id = get_device_id()
    if device_id:
        save_device_settings(device_id, api_key=api_key)

def is_authenticated():
    """JSONBin에서 기기별 인증 상태 확인"""
    device_id = get_device_id()
    if device_id:
        settings = get_device_settings(device_id)
        return settings.get('authenticated', False)
    return False

def save_authenticated():
    """JSONBin에 기기별 인증 상태 저장"""
    device_id = get_device_id()
    if device_id:
        save_device_settings(device_id, authenticated=True)
...
# localStorage 사용 (브라우저별 기기 코드 저장용)
def save_device_id_to_browser(device_id):
    """기기 코드를 브라우저 localStorage에 저장"""
    if JS_AVAILABLE:
        try:
            st_javascript(f"localStorage.setItem('writey_device_id', '{device_id}')")
        except:
            pass

def load_device_id_from_browser():
    """브라우저 localStorage에서 기기 코드 로드"""
    if JS_AVAILABLE:
        try:
            result = st_javascript("localStorage.getItem('writey_device_id')")
...
def get_device_id():
    """기기 코드 반환 (세션 > localStorage 순서로 확인)"""
    # 1. 세션에 있으면 반환
    if 'browser_device_id' in st.session_state and st.session_state['browser_device_id']:
        return st.session_state['browser_device_id']
    # 2. localStorage에서 로드 시도
    browser_device_id = load_device_id_from_browser()
    if browser_device_id:
        st.session_state['browser_device_id'] = browser_device_id
        return browser_device_id
    return None
...
def save_device_settings(device_id, api_key=None, password=None, authenticated=None):
    """기기별 설정을 JSONBin에 저장 (API 키, 비밀번호, 인증 상태)"""
    data = get_jsonbin_data()
    if data:
        ...
        if authenticated is not None:
            data["device_settings"][device_id]["authenticated"] = authenticated
        return update_jsonbin_data(data)
    return False
...
if 'authenticated' not in st.session_state:
    st.session_state['authenticated'] = is_authenticated() and is_device_approved_jsonbin()
        ...
        if not device_id:
            ...
            with tab1:
                ...
                if st.button("확인", key="btn_manual_id"):
                    if manual_id:
                        ...
                        save_device_id_to_browser(manual_id)  # localStorage에 저장
                        st.rerun()
            ...
            with tab2:
                ...
                if st.button("이 코드로 등록 요청", key="btn_new_device"):
                    ...
                    save_device_id_to_browser(new_code)  # localStorage에 저장
        ...
        # 기기 코드가 있고 승인됨
            ...
            if st.button("입장", key="btn_login"):
                if pw == CORRECT_PASSWORD:
                    st.session_state['authenticated'] = True
                    if remember_pw:
                        save_device_settings(device_id, password=pw, authenticated=True)
                    st.rerun()
                else:
...
with st.sidebar:
    ...
    if 'claude_model' not in st.session_state:
        st.session_state['claude_model'] = "claude-opus-4-5-20251101"

    model_options = {
        "Claude Opus 4.5 (추천)": "claude-opus-4-5-20251101",
...
def ask_ai(prompt, temp=0.7):
    ...
    # 선택된 모델 가져오기 (기본값: Opus 4.5)
    model = st.session_state.get('claude_model', 'claude-opus-4-5-20251101')
